...
public partial class Tesoreria_cierreVentas : System.Web.UI.Page
{
    ...
    protected string consultarGastoAutorizado()
    {
       // var consultaGto = new object();
        ...
        if (sucursal == "QTE" || sucursal == "QTF")
        {
            DateTime fecha = Convert.ToDateTime(txtFecha.Text);
            string dia = Convert.ToString(fecha.Day);
            string mes = Convert.ToString(fecha.Month);
            string ano = Convert.ToString(fecha.Year);

            dia = llenarCeros(dia.Trim(), '0', 2);

            lnumero = "S" + sucursal+dia + mes + ano;
        }
        ...
        var estados = new string[] { "1", "2" };
           ...
           var consultaGto = from gto in dc.tbl_CabEgresos
                          ...
                          select new
                          {
                             totalEgreso = gto.totalEgreso,
                             totalRtencion = gto.totalRetencion,
                             totalPagado = gto.totalEgreso - gto.totalRetencion
                          };
