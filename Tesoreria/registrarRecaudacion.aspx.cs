...
public partial class Tesoreria_registrarRecaudacion : System.Web.UI.Page
{
    ...
    #region PROCESOS 
        ...
        protected void ddlTipoPago_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            pnValorEntregado.Visible = false;
            pnTercero.Visible = true;

            if (clave == "1")
            {
                pnDatosDocumento.Visible = false;
            }
            else {
                pnDatosDocumento.Visible = true;
            }
        ...
        protected void btnRegistrar_Click()
        {
            ...
            decimal lrecaudado, lretenidoIVA, lretenidoFUENTE,lValorEntregado;
            ...
            lValorEntregado = Convert.ToDecimal(txtValorEntregado.Text);
            ...
            if (ltipopago == 17 )
            {
                string idTercero = txtTercero.Text.Trim();
                if (idTercero == "")
                {
                    lblRegistro.Text = "Debe especificar identificaciòn del tercero";
                    pasa = false;
                }
            }
            if (ltipopago == 9 )
            {
                if (lValorEntregado <= 0 )
                {
                    lblRegistro.Text = "Debe especificar el valor de transferencia";
                    pasa = false;
                }
            }
        ...
        protected int registrarCabeceraRecaudacion()
        {
            ...
            if (consultaRc.Count() == 0)
            {
                ...
                dc.sp_abmRecaudacionCabecera_v2(laccion, lano, lperiodo, ltipo, lnumero, lsucursal, lfecha, lusuario, lfechaActual, 0, DateTime.Now, UsuarioID, DateTime.Now, UsuarioID,DateTime.Now,true);
            }
            else
            {
                foreach (var registro in consultaRc)
        ...
        protected void registrarDetalleRecaudacion(int lid)
        {
            ...
            decimal lvalor, lretencionIVA, lretencionFUENTE,lValorEntregado;
            DateTime lfecha, lfechaActual;
            laccion = "AGREGAR";
            lfecha = Convert.ToDateTime(txtFechaIni.Text + " 00:00:00");
            lfechaActual = DateTime.Now;
            ...
            int lid_usuario = Convert.ToInt32(Session["SUsuarioID"]) ;

            decimal numeroRetencion = Convert.ToDecimal(txtRetencion.Text);
            ...
            /*DETALLE RECAUDACION*/
            /*dc.sp_abmRecaudacionDetalle3(laccion, lid, lidDet, lano, lperiodo, ltipo, lnumero, lsucursal, lfactura,
                                                lregistro, lvalor, lretencionIVA, lretencionFUENTE,
                                                ltipopago, ldescricionTipoPago, ltipoDetalle, ldetalle, lnumeroDocumento, lTercero, lValorEntregado);*/


            dc.sp_abmRecaudacionDetalle4(laccion,0,lid,lano,lperiodo,ltipo,lnumero,lsucursal,lfactura,lregistro,lvalor,lretencionFUENTE,lretencionIVA,ltipopago,ldescricionTipoPago
                                        , ltipoDetalle, ldescricionTipoPago, lnumeroDocumento, provisionado, 0, lTercero, lValorEntregado,ldetalle,ltipoDetalle,lid_usuario, DateTime.Now, 0, DateTime.Now, true
                                        ,numeroRetencion,autorizacion,fechaEmision,fechaCaducidad);
    ...
    protected void btnIngRetencion_Click(object sender, EventArgs e)
    {
        divRetencion.Visible = true;
    }
}