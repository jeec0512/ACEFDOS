...
public partial class Egreso_controlEgresosRetencion : System.Web.UI.Page
{
    ...
    protected void registrarCabeceraEgresos()
    {
        ...
        if (valorId == 0)
        {
            ...
            //dc.abmEgresosCabecera2(lAccion, lid_CabEgresos, lnumero, lsucursal, lfecha, ldescripcion, lestado, lusuario, "", 0);
        }
    ...
    protected void btnConsultar_Click()
    {
        ...
        if (lsucursal == "QTE" || lsucursal == "GUM")
        {
            //var cEgresos = dc.sp_ListarDetalleGastoRetencion_v2(lAccion, lsucursal, lFechaIni, lFechaFin, caja, 0);

            var cEgresos = dc.sp_ListarGastosyRetenciones_V2(lAccion, lsucursal, lFechaIni, lFechaFin, caja, 0);
            pnDetalleCaja_v2.Visible = true;
            pnDetalleCaja.Visible = false;
            grvEgresosDetalle_v2.DataSource = cEgresos;
            grvEgresosDetalle_v2.DataBind();

            var ctotEgresos = dc.sp_valorAPagarCajaEgresos(lsucursal, lFechaIni, lFechaFin, caja);
            grvTotales_v2.Visible = true;
            pnDetalleCaja.Visible = false;
            grvTotales_v2.DataSource = ctotEgresos;
            grvTotales_v2.DataBind();
        ...
        {
            var cEgresos = dc.sp_ListarGastosyRetenciones(lAccion, lsucursal, lFechaIni, lFechaFin, caja, 0);
            pnDetalleCaja_v2.Visible = false;
    ...
    protected void grvEgresosDetalle_v2_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if ((e.Row.RowType == DataControlRowType.DataRow) && (e.Row.RowType != DataControlRowType.EmptyDataRow))
        {
            /*  subtotal += Convert.ToDecimal(e.Row.Cells[11].Text);
    ...
    protected void grvEgresosDetalle_v2_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "modReg")
        {
            int indice = Convert.ToInt32(e.CommandArgument);
            GridViewRow row = grvEgresosDetalle_v2.Rows[indice];
            int id_pregunta = row.DataItemIndex;
            int lidCab = Convert.ToInt32(row.Cells[4].Text);
            int lidDet = Convert.ToInt32(row.Cells[5].Text);
        ...
        if (e.CommandName == "verRet_v2")
        {
            ...
            string lruc = row.Cells[3].Text;
            string lnumRetencion = row.Cells[12].Text;
            string lautRetencion = row.Cells[9].Text;
