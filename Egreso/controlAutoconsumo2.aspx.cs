...
public partial class Egreso_controlAutoconsumo2 : System.Web.UI.Page
{
    ...
    protected void llenarListados()
    {
        ...
        ddlBrCodCble.DataSource = cBCble;
    ...
    protected void registrarCabeceraEgresos()
    {
        ...
        if (valorId == 0)
        {
            ...
            dc.sp_abmAutoconsumo(lAccion, lid_CabEgresos, lnumero, lsucursal, lfecha, ldescripcion, lestado, lusuario, "", 0);
        }
    ...
    protected void registrarDetalleEgresos()
    {
        ...
        */
        ...
        string lrepuesto = txtRepuesto.Text.Trim();
        ...
        if (cDet.Count() <= 0)
        {
          ...
          //  dc.sp_abmAutoconsumoDetalle2(lAccion,lid_DetEgresos,lid_CabEgresos,id_Concepto,lruc,lnombres,lid_documento,ldocumento,lnumAutorizacionRetencion,lautorizacion,;
    ...
    protected void llenarDetalle(int idDet)
    {
        ...
        var cDetEgreso = from vDetEgreso in dc.tbl_DetEgresos
                         ...
                         select new
                         {
                            ...
                            ,rmae_gas = vDetEgreso.rmae_gas
                            ,rvar_gen = vDetEgreso.rvar_gen
                            ,descripRepuesto = vDetEgreso.descripRepuesto
                            ,subtotalRepuesto = vDetEgreso.subtotalRepuesto
                            ,tarifaCeroRepuesto = vDetEgreso.tarifaCeroRepuesto
                            ,otroRepuesto = vDetEgreso.otroRepuesto
                            ,ivaRepuesto = vDetEgreso.ivaRepuesto
                            ,totalRepuesto = vDetEgreso.totalRepuesto
                            ,
                         };
        ...
        else
        {
            foreach (var registro in cDetEgreso)
            {
                ...
                txtDocumento.Text = registro.numDocumento;
                ...
                /**/
                //ddlBrGastos.SelectedValue = registro.rmae_gas;
                //ddlBrCodCble.SelectedValue = registro.rvar_gen;
                txtRepuesto.Text = registro.descripRepuesto;
                txtRsubtotal.Text = Convert.ToString(registro.subtotalRepuesto);
                txtRtarifa0.Text = Convert.ToString(registro.tarifaCeroRepuesto);
                txtRotros.Text = Convert.ToString(registro.otroRepuesto);
                txtRIva.Text = Convert.ToString(registro.ivaRepuesto);
                txtRtotal.Text = Convert.ToString(registro.totalRepuesto);
    ...
    protected void btnConsultar_Click()
    {
        ...
        try
        {
            ...
            var cEgresos = dc.sp_ListarAutoconsumos2(lAccion, lsucursal, lFechaIni, lFechaFin, "", 0);
    ...
    protected void grvEgresosDetalle_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if ((e.Row.RowType == DataControlRowType.DataRow) && (e.Row.RowType != DataControlRowType.EmptyDataRow))
        {
            ...
            fuente += Convert.ToDecimal(e.Row.Cells[26].Text);
            iva += Convert.ToDecimal(e.Row.Cells[27].Text);
            totalretenido += Convert.ToDecimal(e.Row.Cells[28].Text);
            apagar += Convert.ToDecimal(e.Row.Cells[29].Text);
        }
        if (e.Row.RowType == DataControlRowType.Footer)
        {
            ...
            e.Row.Cells[23].Text = Convert.ToString(fuente);
            e.Row.Cells[24].Text = Convert.ToString(iva);
            e.Row.Cells[25].Text = Convert.ToString(totalretenido);
            e.Row.Cells[26].Text = Convert.ToString(apagar);
        }
    }
