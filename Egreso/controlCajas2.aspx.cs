...
public partial class Egreso_controlCajas2 : System.Web.UI.Page
{
    ...
    protected void btnConsultar_Click()
    {
        ...
        if (lsucursal == "QTE" || lsucursal == "_TV")
        {
            var cEgresos = dc.sp_conmutarControlEgresos(lAccion, lsucursal, lFechaIni, lFechaFin, caja, 0);
    ...
    protected void grvEgresosDetalle_v2_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName == "verRet_v2")
        {
            ...
            //int lidRet = Convert.ToInt64(row.Cells[6].Text);

            string lruc = row.Cells[3].Text;
            string lnumRetencion = row.Cells[12].Text;
            string lautRetencion = row.Cells[9].Text;
            string lidRet = row.Cells[13].Text;


            Session["pRetencion"] = lnumRetencion;
            Session["pSuc"] = lsuc;
            Session["pIdRetencion"] = lidRet;

            // Page.ClientScript.RegisterStartupScript(this.GetType(), "Scripts", "<script>window.open('~/cierre.aspx','popup','width=800,height=500') </script>");

            Page.ClientScript.RegisterStartupScript(this.GetType(), "Scripts", "<script>window.open('imptimirRetencion_v2.aspx','','width=800,height=750') </script>");
