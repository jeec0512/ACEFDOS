...
public partial class Tributacion_sinRet : System.Web.UI.Page
{
    ...
    {
        /* grvRetSuc.DataSource = retornaTodos();
         grvRetSuc.DataBind();*/
        ...
        grvRetSuc.DataSource = dc.sp_EgresosSuc("DETALLE", lsucursal, lfechainicio, lfechafin);
        grvRetSuc.DataBind();
    }
    ...
    protected void btnConsolidado_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            grvRetSuc.DataSource = dc.sp_EgresosTodos("DETALLE", lsucursal, lfechainicio, lfechafin);
            grvRetSuc.DataBind();

            //grvRetSuc.DataSource = dc.sp_ats4_todos("DETALLE", lsucursal, lfechainicio, lfechafin);
            //grvRetSuc.DataBind();
            ...
            /*if (tipo == 4)
            {
                grvRetNac.DataSource = df.sp_facturacionxfechaxsucursalxtipo("CONSOLIDADO", "", ltipoConcepto, lfechainicio, lfechafin);
                grvRetNac.DataBind();
            ...
            {
                grvRetNac.DataSource = df.sp_facturacionxfechaxsucursalxtipo("XSUCURSAL", lsucursal, ltipoConcepto, lfechainicio, lfechafin);
                grvRetNac.DataBind();
            }*/
    ...
    {
        grvRetSuc.DataSource = retornaLista();
        grvRetSuc.DataBind();
    }
    ...
    protected void grvRetSuc_SelectedIndexChanging(object sender, GridViewSelectEventArgs e)
    {

    }

    protected void grvRetSuc_PageIndexChanged(object sender, EventArgs e)
    {

    }
    protected void grvRetSuc_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
        // int tipoConsulta = Convert.ToInt16(lblTipoConsulta.Text);

        grvRetSuc.PageIndex = e.NewPageIndex;
        // if (tipoConsulta == 1)
        // {
        cargaGrid();
        //}
    ...
    protected void uno()
    {
        ...
        using (StringWriter sw = new StringWriter())
        {
            ...
            //To Export all pages
            grvRetSuc.AllowPaging = false;
            ///this.retornaTodos();

            grvRetSuc.HeaderRow.BackColor = Color.White;
            foreach (TableCell cell in grvRetSuc.HeaderRow.Cells)
            {
                cell.BackColor = grvRetSuc.HeaderStyle.BackColor;
            }
            foreach (GridViewRow row in grvRetSuc.Rows)
            {
                ...
                foreach (TableCell cell in row.Cells)
                {
                    if (row.RowIndex % 2 == 0)
                    {
                        cell.BackColor = grvRetSuc.AlternatingRowStyle.BackColor;
                    }
                    else
                    {
                        cell.BackColor = grvRetSuc.RowStyle.BackColor;
                    }
                    cell.CssClass = "textmode";
                }
            }

            grvRetSuc.RenderControl(hw);
    ...
    protected void dos()
    {
        ...
        using (StringWriter sw = new StringWriter())
        {
            ...
            //To Export all pages
            grvRetNac.AllowPaging = false;
            /// this.BindGrid();

            grvRetNac.HeaderRow.BackColor = Color.White;
            foreach (TableCell cell in grvRetNac.HeaderRow.Cells)
            {
                cell.BackColor = grvRetNac.HeaderStyle.BackColor;
            }
            foreach (GridViewRow row in grvRetNac.Rows)
            {
                row.BackColor = Color.White;
                foreach (TableCell cell in row.Cells)
                {
                    if (row.RowIndex % 2 == 0)
                    {
                        cell.BackColor = grvRetNac.AlternatingRowStyle.BackColor;
                    }
                    else
                    {
                        cell.BackColor = grvRetNac.RowStyle.BackColor;
                    }
                    cell.CssClass = "textmode";
                }
            }

            grvRetNac.RenderControl(hw);
