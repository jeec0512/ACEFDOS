...
public partial class Catalogo_aula : System.Web.UI.Page
{
    ...
    protected void ibConsultar_Click(int id)
    {
        ...
        foreach (var registro in cAula)
        {
            ...
            txtCapacidad.Text = Convert.ToString(registro.AUL_CAPACIDAD);
    ...
    protected void aulaExiste(int lidAula)
    {
        int aula = 0;
        var cAul = from mAUl in ds.TB_ASIGNA_MATERIA
                   where mAUl.AUL_ID == lidAula
                   select new
                   {
                       AUL_ID = mAUl.AUL_ID
                   };
        foreach (var regNum in cAul)
        {
            aula = Convert.ToInt32(regNum.AUL_ID);
        };

        if (aula > 0)
        {
            throw new InvalidOperationException("La aula está creada en cupos");
        }
    }
